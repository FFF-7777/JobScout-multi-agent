...
def fetch_research_context(
    ...
    *,
    ...
    system = (
        ...
        "给出与岗位技术语境强相关的研究摘要。若存在公司名，同时简要查询公司背景、"
        "主营业务与当前岗位的关联，以及有可靠公开来源支持的明显招聘风险。"
        "来源优先级依次为公司官网、政府或监管公开信息、权威媒体、招聘官方页面；"
        "论坛、聚合内容和员工评价只能作为待核实线索，不能当成确定事实。"
        "必须结合公司名称、城市、主营业务和岗位交叉确认，避免把同名公司的信息混入。"
        "公司信息必须以公开来源为依据；信息不足时省略，不得猜测。"
        "summary_items 与 source_notes 按相同顺序对应，并区分公开事实、合理推断和无法核实的信息。"
        "不要重复岗位原文；只输出 JSON。"
    )
    user = json.dumps(
        {
            ...
            "goal": (
                "总结该岗位在技术栈、工程要求、常见评估重点上的外部语境；"
                "有可靠来源时，补充公司主营业务、与岗位关联及明显风险，信息不足则不写。"
            ),
            "output_schema": {
                "summary_items": ["3-6 条研究摘要，可包含有来源支撑的公司背景、岗位关联或风险提示"],
                "source_notes": ["对应摘要的简短来源说明；无法核实时为空"],
            },
        },
        ensure_ascii=False,
    )
